...
def find_all_length(bob_num = int(raw_input("Enter number of bobs: ")), longest_length = float(raw_input("Enter longest string length: ")), cycle = float(raw_input("Enter number of cycles: "))):
    len_list = []
...
'''    if (fnum >= 1000):
        break
    elif (ic%18 == 0):      # grab every 20 iterations, may need adjustment
        im = ImageGrab.grab((0,0,1600,1600))
        num = '00'+repr(fnum)           # sequence num 000-00999, trunc. below
        im.save('img-'+num[-4:]+'.png') # save to png file, 000-999, 3 digits
        fnum += 1
    ic += 1
